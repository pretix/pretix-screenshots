...
def cart(organizer, event, tax_rule, live_server, logged_in_client):
    quota_tickets = event.quotas.create(name=_('Tickets'), size=5)
...
def shot_shipping_checkout(live_server, organizer, event, logged_in_client, shipping_method_letter,
                           shipping_method_online, cart):
